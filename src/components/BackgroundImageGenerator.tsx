...
export default function BackgroundImageGenerator({ onGenerate, isLoading, preferenceAssets = [] }: BackgroundImageGeneratorProps) {
    ...
    <Card>
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="bg-image-upload">Upload Images (Optional, Multiple Allowed)</Label>
            {(uploadedImages.length > 0 || selectedPreferences.length > 0) ? (
              <div className="space-y-4">
                <div className="space-y-2">
                  <Label>Uploaded Images ({totalSelectedImagesCount})</Label>
                  <div className="grid grid-cols-3 gap-2">
                    {[...uploadedImages, ...selectedPreferences].map((img, index) => (
                    ...
                    <PopoverContent className="w-64">
                      <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <label htmlFor="bg-image-upload-more" className="flex flex-col items-center justify-center p-3 border rounded-md cursor-pointer hover:bg-muted/50 transition-colors">
                            <FolderOpen className="w-6 h-6 mb-1 text-muted-foreground" />
                            <span className="text-xs">From Device</span>
                            <Input id="bg-image-upload-more" type="file" className="hidden" onChange={handleImageChange} accept="image/png, image/jpeg, image/webp" disabled={isLoading} multiple={true} />
                          </label>
                          <label htmlFor="bg-camera-upload-more" className="flex flex-col items-center justify-center p-3 border rounded-md cursor-pointer hover:bg-muted/50 transition-colors">
                            <Camera className="w-6 h-6 mb-1 text-muted-foreground" />
                            <span className="text-xs">Camera</span>
                            <Input id="bg-camera-upload-more" type="file" accept="image/*" capture="environment" className="hidden" onChange={handleImageChange} disabled={isLoading} />
                          </label>
                          <button 
                          ...
                          <label htmlFor="bg-selfie-upload-more" className="flex flex-col items-center justify-center p-3 border rounded-md cursor-pointer hover:bg-muted/50 transition-colors">
                            <ImageIcon className="w-6 h-6 mb-1 text-muted-foreground" />
                            <span className="text-xs">Selfie</span>
                            <Input id="bg-selfie-upload-more" type="file" accept="image/*" capture="user" className="hidden" onChange={handleImageChange} disabled={isLoading} />
                          </label>
                        </div>
                      </div>
                    </PopoverContent>
                    ...
                    <Popover>
                        ...
                        <PopoverContent className="w-64">
                            <div className="space-y-2">
                                <h4 className="font-medium">Upload Options</h4>
                                <div className="grid grid-cols-2 gap-2">
                                    <label htmlFor="bg-image-upload" className="flex flex-col items-center justify-center p-3 border rounded-md cursor-pointer hover:bg-muted/50 transition-colors">
                                        <FolderOpen className="w-6 h-6 mb-1 text-muted-foreground" />
                                        <span className="text-xs">From Device</span>
                                        <Input id="bg-image-upload" type="file" className="hidden" onChange={handleImageChange} accept="image/png, image/jpeg, image/webp" ref={fileInputRef} disabled={isLoading} multiple={true} />
                                    </label>
                                    <label htmlFor="bg-camera-upload" className="flex flex-col items-center justify-center p-3 border rounded-md cursor-pointer hover:bg-muted/50 transition-colors">
                                        <Camera className="w-6 h-6 mb-1 text-muted-foreground" />
                                        <span className="text-xs">Camera</span>
                                        <Input id="bg-camera-upload" type="file" accept="image/*" capture="environment" className="hidden" onChange={handleImageChange} disabled={isLoading} />
                                    </label>
                                    <button 
                                        className="flex flex-col items-center justify-center p-3 border rounded-md cursor-pointer hover:bg-muted/50 transition-colors"
                                        onClick={() => {
                                    ...
                                    <label htmlFor="bg-selfie-upload" className="flex flex-col items-center justify-center p-3 border rounded-md cursor-pointer hover:bg-muted/50 transition-colors">
                                        <ImageIcon className="w-6 h-6 mb-1 text-muted-foreground" />
                                        <span className="text-xs">Selfie</span>
                                        <Input id="bg-selfie-upload" type="file" accept="image/*" capture="user" className="hidden" onChange={handleImageChange} disabled={isLoading} />
                                    </label>
                                </div>
                            </div>
                        </PopoverContent>
          ...
          <Button type="submit" disabled={isLoading || (!prompt.trim() && uploadedImages.length === 0)} className="w-full sm:w-auto">
            {isLoading ? (
              <>
